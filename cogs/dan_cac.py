...
class DanCacSelect(discord.ui.Select):
    def __init__(self, items_with_price):
        ...
        super().__init__(
            placeholder="Chọn loại đan dược để mua...",
            min_values=1,
            max_values=1,
            options=options,
        )
...
class DanCacView(discord.ui.View):
    ...
    async def process_purchase(self, interaction: discord.Interaction, item_id: int):
        ...
        async with get_db(self.db_path) as db:
            ...
            if not row:
                return await interaction.response.send_message(
                    "❌ Ngài chưa có tên trong hệ thống tu chân!", ephemeral=True
                )

            player_lt = row[0]
            if player_lt < price:
                return await interaction.response.send_message(
                    f"⚠️ Kém cỏi! Ngài chỉ có {player_lt:,} Linh thạch, không đủ {price:,} nghìn lấy gì mua?",
                    ephemeral=True,
                )
        ...
        await interaction.response.send_message(
            f"🛒 **GIAO DỊCH THÀNH CÔNG!** {interaction.user.mention} đã mua thành công 1 viên **{name}** với giá {price:,} Linh Thạch."
        )
...
class DanCac(commands.Cog):
    ...
    async def dancac(self, ctx):
        """Mở giao diện mua đan dược tại Đan Các."""
        async with get_db(self.db_path) as db:
            cursor = await db.execute(
                "SELECT item_id, ten_vat_pham FROM item_master WHERE loai_vat_pham = 'dan_duoc' ORDER BY item_id ASC"
            )
        ...
        if not items_with_price:
            return await ctx.send(
                "🏮 **Đan Các** thông báo: Hiện tại tệ điếm đã hết linh đan, xin quay lại sau!"
            )

        view = DanCacView(self.db_path, items_with_price)
        embed = discord.Embed(
            title="🏯 QUANG LÂM ĐAN CÁC",
            description=f"Kính chào đạo hữu {ctx.author.mention}, đây là các loại linh đan hỗ trợ tu luyện và đột phá. Hãy lựa chọn loại đan muốn mua:",
            color=discord.Color.orange(),
        )
        embed.set_footer(text="Dược tính mạnh, xin hãy dùng đúng lúc!")
