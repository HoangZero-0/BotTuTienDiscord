...
class SuKien(commands.Cog):
    ...
    async def chemboss(self, ctx):
        ...
        async with get_db(self.db_path) as db:
            ...
            c = await db.execute(
                """
                SELECT p.the_luc, p.luc_chien_goc + (SELECT IFNULL(SUM(im.chi_so_buff), 0) FROM inventory i JOIN item_master im ON i.item_id = im.item_id WHERE i.user_id = p.user_id AND i.trang_thai = 'dang_trang_bi') 
            ...
            the_luc, cp = res
            if the_luc < 1:
                return await ctx.send("⚠️ Thể lực đã cạn, không thể chém Boss!")

            damage = int(cp * random.uniform(0.7, 1.3))
            damage = max(1, damage)

            # Trừ thể lực
            await db.execute(
                "UPDATE players SET the_luc = the_luc - 1 WHERE user_id = ?", (user_id,)
            )
